...
//                    }
                ...
                self.handleAttached = function (info) {
                	...
                	$.ajax({
	                	    dataType: "json",
	                	    type: 'post',
	                	    url: "http://localhost:8080/api-1.0-SNAPSHOT/v1/assessments/getDiagramData",
	                	    data: "{\"timestampStart\":\"2016-01-01 00:00:00\",\"timestampEnd\":\"2017-01-01 00:00:00\",\"crId\":1,\"dvParentId\":4}",
	                	    contentType: "application/json; charset=utf-8",
	                	    success: function(data) {
	                                 	
                        	function Series(name, color, source, items, lineType, markerDisplayed, markerSize) {
                        		this.name = name;
                        		this.color = color;
                        		this.source = source;
                        		this.items = items;
	                	    ...
	                	    for (i = 0; i < countGefs; i++) {
	                	    	...
	                	    	series[i] = new Series(data.gefLabels[i], null, null, nullfill, null, null, null);
	                	    }
	                	    	...
	                	    	for (j = 0; j < groups.length; j++) {
	                	    		...
	                	    		for (k = 0; k < data.gefs.length; k++) {    	    			
	                	    			 if((data.gefs[k].cdDetectionVariable.detectionVariableName == series[i].name) && (data.gefs[k].timeInterval.start == groups[j])) {
	                	    					 series[i].items[j] = data.gefs[k].gefValue;	                	    					 
	                	    			 } 
	                	    		 }
	                	    	}
	                	    }
		                	...
		                	$.ajax({
			                	    dataType: "json",
			                	    type: 'post',
			                	    url: "http://localhost:8080/api-1.0-SNAPSHOT/v1/assessments/getLastFiveAssessmentsForDiagram",
			                	    data: "{\"timestampStart\":\"2016-01-01 00:00:00\",\"timestampEnd\":\"2017-01-01 00:00:00\",\"crId\":1}",
			                	    contentType: "application/json; charset=utf-8",
			                	    success: function(data) {		                	  
				                	    ...
				                	    series.push(new Series("Alerts", '#e83d17', "images/alert.png", nullfill, 'none', 'on', 20));
				                	    series.push(new Series("Warnings", '#ffff66', "images/warning-icon.png", nullfill, 'none', 'on', 20));
				                	    series.push(new Series("Comments", '#ebebeb', "images/comment-gray.png", nullfill, 'none', 'on', 20));
