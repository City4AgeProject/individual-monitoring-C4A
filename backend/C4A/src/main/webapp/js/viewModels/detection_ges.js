define(
		[ 'ojs/ojcore', 'knockout', 'jquery', 'setting_properties',
				'ojs/ojknockout', 'ojs/ojmodule', 'ojs/ojmodel', 'ojs/ojchart',
				'ojs/ojlegend', 'ojs/ojbutton', 'ojs/ojmenu', 'ojs/ojpopup',
				'ojs/ojinputtext', 'ojs/ojtoolbar', 'ojs/ojselectcombobox',
				'ojs/ojslider', 'ojs/ojradioset', 'ojs/ojdialog',
				'ojs/ojlistview', 'ojs/ojarraytabledatasource', 'ojs/ojswitch',
				'ojs/ojtabs', 'urls', 'entities' ],
		function(oj, ko, $, sp) {
			...
			function detectionGesContentViewModel() {
				...
				var loadDiagramDataCallback = function(data) {
					...
					$.each(data.itemList, function(i, list) {
						...
						$.each(list.items[0].itemList, function(j, itemList) {
							nodes.push(createItems(list.items[0].idList[j],
									itemList));
						});
					...
					$.each(data.itemList[0].items[0].dateList, function(j,
							dateItem) {
				...
				var loadDataSet = function(data) {
					...
					// JSON.stringify({timestampStart:'2016-01-01
					// 00:00:00',timestampEnd:'2017-01-01
					// 00:00:00',crId:1,dvParentId:4})
					// , loadSucessCallback);
					// jqXHR.fail(serverErrorCallback);

					var jqXHR = $.getJSON(url + "?careReceiverId="
							+ self.careReceiverId() + "&parentFactorId="
							+ self.parentFactorId(), loadDiagramDataCallback);
					jqXHR.fail(serverErrorCallback);
					return jqXHR;
				};
							...
							.getJSON(
									...
									function(assessments) {
										...
										for (var i = 0; i < assessments.length; i++) {
											...
											newAssessment.from = assessment.userInRole.userInSystem.displayName;
										...
										self
												.selectedAnotations(assessmentsResult);
										self.dataPointsMarked(self
												.dataPointsMarked()
												+ ' with '
				...
				var loadAssessmentsCached = function(ids) {
					...
					return $
							.getJSON(
									ASSESSMENT_LAST_FIVE_FOR_INTERVAL
											+ '/userInRoleId/'
											+ self.careReceiverId()
											+ '/intervalStart/2011-1-1/intervalEnd/2017-1-1',
									function(dataSet) {
										// insert to quick read later on mouse
										// over popup
										var assesmentsDataSet = DataSet
												.produceFromOther(dataSet);
										// var assessments =
										// assesmentsDataSet.getAssessments();
										var assessmentsSerieAlerts = Serie
												.produceAlert();
				...
				/*
				 * handleAttached; Use to perform tasks after the View is
				 * inserted into the DOM., str 103
				 */
				...
				/*
				 * Mouse handles .. should be deleted when we found better way
				 * to fix popup position
				 */
				var clientX;
				var clientY;
				...
				function showAssessmentsPopup() {
					...
					$('#popup1').ojPopup('open');
					$("#popup1").ojPopup("widget").css("left",
							clientX + 2 + "px");
					$("#popup1").ojPopup("widget").css("top",
							clientY + 2 + "px");
				}
				/**
				 * 
				 * @param {type}
				 *            dataSelection this is ui['optionMetadata'] for
				 *            selected value(s)
				 * @returns {Array} Array of id-s for selected points of chart.
				 *          <br/>
				 * 
				 */
				function filteredSelectionBetweenAssessmentSeriesAndOtherPoints(
						dataSelection) {
					...
					if (ui['option'] === 'selection') {
						if (ui['value'].length > 0) {
							...
							if (onlyDataPoints.length === 0)
								...
								queryParams = calculateSelectedIds(onlyDataPoints);
				...
				self.clickFilter = function() {
					//alert("cfvd: " + checkedFilterValidityData);
					// loadAssessments(queryParams);
				}
				...
				self.postAssessment = function(data, event) {
					...
					var geriatricFactorValueIds = ko
							.toJS(self.dataPointsMarkedIds);
					var audienceIds = ko.toJS(self.selectedRoles);
					var assessmentToPost = new AddAssessment(authorId, comment,
							riskStatus, dataValidityStatus,
							geriatricFactorValueIds, audienceIds);
				...
				/* Risks select */
				self.riskStatusesURL = CODEBOOK_SELECT_ALL_RISKS;
									...
									success : function(collection, response,
										...
										if (self.risksTags.length === 0) {
											for (var i = 0; i < collection
													.size(); i++) {
												var riskModel = collection
														.at(i);
												self.risksTags
														.push({
															value : riskModel.attributes.riskStatus,
				...
				// });
				//                
				// self.dataValiditiesCollection(new
				// collectionDataValidities());
				...
				// self.dataValiditiesTags.push({value:
				// dataValidityModel.attributes.dataValidity, label:
				// dataValidityModel.attributes.dataValidityDesc, imagePath:
				// dataValidityModel.attributes.imagePath});
				// }
				// }
				// },
				...
				/* Audience ids -> CdRole */
				self.rolesForStakeHoldersURL = CODEBOOK_SELECT_ROLES_FOR_STAKEHOLDER;
				...
				/*
				 * polar chart - uradjen za prvu grupu i to za mesece M1, M2 i
				 * M5
				 */
				var groups = [ "Initial", "Jan 2016", "Feb 2016", "Mar 2016",
						"Apr 2016", "May 2016", "Jun 2016", "Jul 2016",
						"Avg 2016", "Sep 2016", "Oct 2016", "Nov 2016",
						"Dec 2016" ];
				var series = [
						{
							name : "Walking",
							items : [ 3.0, 1.5, 1.0, 2.2, 1.8, 3.1, 3.0, 3.6,
									2.0, 2.5, 1.5, 3.8, 4.4 ]
						},
						{
							name : "Climbing stairs",
							items : [ 3.0, 4.2, 2.8, 2.2, 3.3, 2.8, 2.8, 1.9,
									2.5, 3.4, 4.1, 2.7, 2.3 ]
						},
						{
							name : "Still/Moving",
							items : [ 3.0, 5.0, 3.7, 4.6, 4.5, 5.0, 4.8, 4.4,
									3.9, 3.9, 5.0, 5.0, 5.0 ]
						},
						{
							name : "Moving across rooms",
							items : [ 3.0, 3.3, 3.8, 5.0, 4.5, 3.9, 3.7, 3.5,
									4.1, 4.0, 3.6, 5.0, 4.5 ]
						},
						{
							name : "Gait balance",
							items : [ 3.0, 2.8, 2.8, 3.2, 2.9, 3.3, 2.7, 2.5,
									3.0, 1.9, 2.3, 1.8, 2.6 ]
						},
						{
							name : "Alerts",
							...
							items : [ null, 1.5, 1.0, null, null, null, null,
						...
						},
						{
							...
							items : [ null, null, null, null, 1.8, null, null,
									1.9, null, 1.9, null, 1.8, null ],
							lineType : 'none',
							markerDisplayed : 'on',
							markerSize : 20
						},
						{
							name : "Comments",
							color : '#ebebeb',
							source : "images/comment-gray.png",
							items : [ null, null, 2.8, null, null, null, null,
									null, null, null, null, 2.7, null ],
							lineType : 'none',
							markerDisplayed : 'on',
							markerSize : 20
						} ];
				var lineSeriesPolar = [
						{
							name : groups[1],
							items : [ series[0].items[1], series[1].items[1],
									series[2].items[1], series[3].items[1],
									series[4].items[1] ],
							color : '#ED6647'
						},
						{
							name : groups[2],
							items : [ series[0].items[2], series[1].items[2],
									series[2].items[2], series[3].items[2],
									series[4].items[2] ],
							color : '#8561C8'
						},
						{
							name : groups[5],
							items : [ series[0].items[5], series[1].items[5],
									series[2].items[5], series[3].items[5],
									series[4].items[5] ],
							color : '#6DDBDB'
						} ];
				var series1 = [
				...
				var lineGroupsPolar = series1; // grupe su nazivi serija
				// linijskog dijagrama bez
				// alerta

				self.stackValue = ko.observable('off');
				self.typeValue = ko.observable('line');
				self.polarGridShapeValue = ko.observable('polygon');
				self.polarChartSeriesValue = ko
						.observableArray(lineSeriesPolar);
				self.polarChartGroupsValue = ko
						.observableArray(lineGroupsPolar);
			}
