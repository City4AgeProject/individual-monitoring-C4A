...
public interface TimeIntervalRepository extends GenericRepository<TimeInterval, Long> {
	...
	@Query("SELECT DISTINCT ti FROM TimeInterval ti "
			+ "LEFT JOIN FETCH ti.frailtyStatusTimeline AS fst "
			+ "LEFT JOIN FETCH ti.geriatricFactorValue AS gfv "
			+ "INNER JOIN FETCH gfv.detectionVariable AS dv "
			+ "LEFT JOIN FETCH dv.detectionVariableType AS dvt "
			+ "INNER JOIN FETCH gfv.userInRole AS uir "
			+ "INNER JOIN FETCH uir.userInSystem AS uis "
			+ "WHERE (uir.id = :userId OR uir IS NULL) "
			+ "AND (dvt.detectionVariableType IN :gefType OR dv IS NULL) "
