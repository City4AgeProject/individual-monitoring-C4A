...
@EntityFilter(filterQueries = {
		@FilterQuery(name = "findForSelectedDataSet", 
				jpql = "SELECT aa FROM Assessment aa "
						+ "JOIN aa.geriatricFactorValue AS gfv "
						+ "JOIN aa.roles AS aar "
						+ "LEFT JOIN FETCH aa.userInRole AS uir "
						+ "LEFT JOIN FETCH uir.userInSystem AS uis "
						+ "WHERE gfv.id IN :geriatricFactorIds") })
