...
public interface TimeIntervalRepository extends GenericRepository<TimeInterval, Long> {
	...
	@Query("SELECT DISTINCT ti FROM TimeInterval ti LEFT JOIN FETCH ti.frailtyStatusTimeline AS fst LEFT JOIN FETCH ti.geriatricFactorValue AS gfv INNER JOIN FETCH gfv.detectionVariable AS dv LEFT JOIN FETCH dv.detectionVariableType AS dvt INNER JOIN FETCH gfv.userInRole AS uir INNER JOIN FETCH uir.userInSystem AS uis WHERE uir.id = :userId AND (dvt.detectionVariableType IN :gefType OR dv IS NULL) ORDER BY ti.intervalStart")
	...
	@Query("SELECT DISTINCT ti FROM TimeInterval ti LEFT JOIN FETCH ti.geriatricFactorValue AS gfv LEFT JOIN FETCH gfv.detectionVariable AS dv LEFT JOIN FETCH dv.pilotDetectionVariable AS pdv LEFT JOIN FETCH gfv.userInRole AS uir LEFT JOIN FETCH uir.pilot AS p WHERE (uir IS NULL OR pdv IS NULL OR (uir.id = :careRecipientId AND pdv.derivedDetectionVariable.id IN :parentFactorIds AND pdv.pilotCode = uir.pilotCode)) AND (p IS NULL OR ti.typicalPeriod = 'mon' AND DATE_TRUNC('month', TIMEZONE(p.timeZone, ti.intervalStart) ) = TIMEZONE(p.timeZone, ti.intervalStart)) AND ti.intervalStart >= (SELECT min(ti1.intervalStart) FROM TimeInterval ti1 INNER JOIN ti1.geriatricFactorValue AS gfv1 INNER JOIN gfv1.detectionVariable AS dv1 INNER JOIN dv1.pilotDetectionVariable AS pdv1 INNER JOIN gfv1.userInRole AS uir1 INNER JOIN gfv1.userInRole AS uir1 WHERE pdv1.derivedDetectionVariable.id = pdv.derivedDetectionVariable.id AND pdv1.pilotCode = uir1.pilotCode) ORDER BY ti.intervalStart ASC")
	List<TimeInterval> getDiagramDataForUserInRoleId(@Param("careRecipientId") final Long careRecipientId, @Param("parentFactorIds") final List<Long> parentFactorIds);
	...
	/*
	 * 
	 * metod koji treba da dohvati truncate timestampa (start) zadatim parametrom
	 */
	...
	/*
	 * 
	 * metod koji treba da dohvati truncate timestampa (end) zadatim parametrom
	 */
