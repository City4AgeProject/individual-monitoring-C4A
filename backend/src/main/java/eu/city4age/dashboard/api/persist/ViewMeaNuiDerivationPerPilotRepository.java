...
public interface ViewMeaNuiDerivationPerPilotRepository extends GenericRepository<ViewMeaNuiDerivationPerPilot, Long> {
	...
	@Query("SELECT vmn FROM ViewMeaNuiDerivationPerPilot vmn WHERE vmn.pilotCode = :pilotCode AND vmn.derivedNuiName = vmn.meaFormula || CAST('_' AS string) || :detectionVariableName")
