...
public interface VariationMeasureValueRepository extends GenericRepository<VariationMeasureValue, Long> {
	...
	@Query("SELECT DATE_TRUNC('month', TIMEZONE(:timeZone, MIN (ti.intervalStart))) FROM VariationMeasureValue vm INNER JOIN vm.userInRole uir INNER JOIN vm.timeInterval ti WHERE uir.pilotCode = :pilotCode AND ti.intervalStart >= '2016-07-01'")
	...
	@Query ("SELECT DISTINCT uir FROM VariationMeasureValue vmv INNER JOIN vmv.userInRole uir INNER JOIN vmv.vmvFiltering vmf INNER JOIN vmf.assessment aa INNER JOIN aa.userInRole uir2 WHERE vmf.validFrom >= :startOfDay AND vmf.filterType = 'E' AND uir2.id <> 2 ORDER BY uir.id ASC")
