...
public interface UserInRoleRepository extends GenericRepository<UserInRole, Long> {
	...
	@Query("SELECT u FROM UserInRole u "
			+ "LEFT JOIN u.crProfile AS crProfile "
			+ "LEFT JOIN u.careProfile AS careProfile "
			+ "LEFT JOIN u.frailtyStatusTimeline AS fst "
			+ "LEFT JOIN fst.cdFrailtyStatus AS cdFrailtyStatus "
			+ "WHERE u.roleId = :roleId "
			+ "AND (fst IS NULL OR fst.changed = (SELECT max(fst.changed) FROM FrailtyStatusTimeline fst WHERE fst.userInRoleId = u.id))")
	...
	@Query("SELECT u FROM UserInRole u "
			...
			+ "LEFT JOIN fst.cdFrailtyStatus AS cdFrailtyStatus "
			+ "WHERE u.roleId = :roleId "
			+ "AND u.pilotCode = :pilotCode "
			+ "AND (fst IS NULL OR fst.changed = (SELECT max(fst.changed) FROM FrailtyStatusTimeline fst WHERE fst.userInRoleId = u.id))")
	List<UserInRole> findByRoleIdAndPilotCode(@Param("roleId") final Short roleId,@Param("pilotCode") final String pilotCode);

	@Query("SELECT u FROM UserInRole u "
			+ "INNER JOIN FETCH u.userInSystem AS userInSystem "
			+ "WHERE userInSystem.username = :username "
			+ "AND userInSystem.password=:password")
