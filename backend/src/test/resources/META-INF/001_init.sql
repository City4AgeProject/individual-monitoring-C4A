...
create table testtest.assessment (id int8 not null, assessment_comment varchar(255), risk_status char(1), data_validity_status char(1), created timestamp , updated timestamp, author_id int8, primary key (id));
create table testtest.assessment_audience_role (assessment_id int8 not null, role_id int8 not null, assigned timestamp , primary key (assessment_id,role_id));
create table testtest.care_profile (user_in_role_id int4 not null, individual_summary varchar(255) , attention_status char(1), intervention_status char(1), last_intervention_date date, created timestamp , last_updated timestamp, created_by int8 , last_updated_by int8, primary key (user_in_role_id));
...
create table testtest.md_pilot_detection_variable (pilot_id int4 not null, detection_variable_id int2 not null, detection_variable_description_formula varchar(255), derived_detection_variable_id int4, valid_from timestamp(6), valid_to timestamp(6), primary key (pilot_id, detection_variable_id));
...
create table testtest.executed_action (id int8 not null, date timestamp , locationid int8 , actionid int8 , activityid int8, userinroleid int8 , rating int4, sensor_id int4 , payload varchar(50) , extra_information varchar(255), primary key (id));
create table testtest.frailty_status_timeline (time_interval_id int8 not null, changed_by int8 not null, changed timestamp , user_in_role_id int8 , frailty_status varchar(9) , frailty_notice varchar(200), primary key (time_interval_id, changed_by));
...
create table testtest.stakeholder (stakeholder_abbreviation varchar(3) not null, stakeholder_name varchar(50) , stakeholder_description varchar(50), valid_from timestamp , valid_to timestamp, primary key (stakeholder_abbreviation));
