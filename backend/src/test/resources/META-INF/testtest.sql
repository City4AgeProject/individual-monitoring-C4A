...
create table testtest.md_pilot_detection_variable (id int8 not null, pilot_code varchar(3), detection_variable_id int2, derivation_function_formula varchar(1000), derived_detection_variable_id int4, valid_from timestamptz(6), valid_to timestamptz(6), derivation_weight numeric(3, 2), primary key (id));
create table testtest.cd_role (id int8 not null, role_name varchar(50), role_abbreviation varchar(3), role_description varchar(200), valid_from timestamptz, valid_to timestamptz, stakeholder_abbreviation varchar(3), primary key (id));
...
create table testtest.source_evidence (geriatric_factor_id int4 not null, author_id int4 not null, text_evidence varchar(255), multimedia_evidence bytea, uploaded timestamptz , primary key (geriatric_factor_id, author_id));
create table testtest.time_interval (id int8 not null, interval_start timestamptz, interval_end timestamptz, typical_period varchar(3), primary key (id), unique (interval_start, typical_period));
create table testtest.user_in_role (id int8 not null, pilot_id int4, pilot_code varchar(3), valid_from timestamptz, valid_to timestamptz, user_in_system_id int4, role_id int2, primary key (id));
create table testtest.user_in_system (id int8 not null, username varchar(25) unique, password varchar(25), created_date timestamptz, display_name varchar(255), primary key (id));
